...
static int testfoo (int args, char *argv[])
{
	rt_kprintf("Hello World! This is my test foo function!\n");
	rt_kprintf(argv[1]);
	rt_kprintf(argv[2]);
	rt_kprintf("\n");
	return 0;
}
MSH_CMD_EXPORT(testfoo, Test Foo Function);

int rt_foo(void) {
	rt_kprintf("Hello World! This is foo, the board init function!\n");
	return 0;
}
INIT_BOARD_EXPORT(rt_foo);
